...
@conformat ROOM=[setq(lkr,%#)][setq(players,[lvplayers(%L,%#)])][setq(things,[lvthings(%l,%#)])][u(conformat`[gameconfig(%#,CONFORMAT)],%q<players>,%q<things>)]
...
&exitformat`lines room=[if(gte(words(%q<paths>),1),[printf($-10s $-[sub([gameconfig(%#,width)],11)]s,[ansi([gameconfig(%#,column_names)],Paths)]:,[u(exitformat`lines`fmt,[sortname(%q<paths>)])])])][if(gte(words(%q<build>),1),[if(gte(words(%q<paths>),1),%R)][printf($-10s $-[sub([gameconfig(%#,width)],11)]s,[ansi([gameconfig(%#,column_names)],Buildings)]:,[u(exitformat`lines`fmt,[sortname(%q<build>)])])])][if(gte(words(%q<paths> %q<build>),1),%R)][line(,%#)]

&exitformat`lines`fmt room=[itemize(iter(%0,[u(get_exit`name,##,%q<lkr>)]%B[ansi([gameconfig(%#,line_color)],[chr(91)])][u(get_exit`alias,##,%q<lkr>)][ansi([gameconfig(%#,line_color)],[chr(93)])],%B,|),|,and)]

&conformat`lines room=[line(,%#)][if(gte(words(%q<players>),1),%R[printf($-10s $-[sub([gameconfig(%#,width)],11)]s,[ansi([gameconfig(%#,column_names)],Players)]:,[u(conformat`lines`players,[sortname(%q<players>)])])])][if(gte(words(%q<things>),1),%R[printf($-10s $-[sub([gameconfig(%#,width)],11)]s,[ansi([gameconfig(%#,column_names)],Objects)]:,[u(conformat`lines`objects,[sortname(%q<things>)])])])]
&conformat`lines`players room=[itemize([iter(%0,[switch(1,[isadmin(##)],[ansi([gameconfig(%#,line_accent)],*)],[isguest(##)],[ansi(+green,+)])][cname(##)][chr(40)][hideidle(##)][chr(41)],%B,|)],|,and)]

&conformat`lines`objects room=[itemize([iter(%0,[if(hasflag(##,PUPPET),[ansi([gameconfig(%#,line_accent)],!)])][cname(##)][if(or(controls(%#,##),isadmin(%#)),[chr(91)]##[chr(93)])],%B,|)],|,and)]

&conformat`default room=[line(,%#)][ifelse(gte(words(%q<players>),1),%R[ansi([gameconfig(%#,line_color)],[chr(91)])][ansi([gameconfig(%#,column_names)],Players)][ansi([gameconfig(%#,line_color)],[chr(93)])]%R[iter([sortname(%q<players>)],[u(conformat`player,##,%#)],%B,%R)])][ifelse(gte(words(%q<things>),1),%R[ansi([gameconfig(%#,line_color)],[chr(91)])][ansi([gameconfig(%#,column_names)],Objects)][ansi([gameconfig(%#,line_color)],[chr(93)])][step(conformat`default`step,[sortname(%q<things>)],2,,)])]

&conformat`default`step room=%R[printf($-39s,[u(conformat`default`thing,%0,%q<lkr>)])]%B%B[printf($-39s,[u(conformat`default`thing,%1,%q<lkr>)])]
&conformat`default`thing room=[if(gte(words(%0),1),[printf($1s $-30s $6s,[if(hasflag(%0,Puppet),[ansi([gameconfig(%#,LINE_ACCENT)],P)])],[cname(%0)],[if(or(controls(%#,%0),isadmin(%#)),%0)])])]

&conformat`player room=[printf($1s $-30s $-[sub([gameconfig(%#,width)],30,1,1,1,1,5)]s $5s,[switch(1,[isadmin(%0)],[ansi([gameconfig(%#,line_accent)],*)],[isguest(%0)],[ansi(+green,G)])],[cname(%0)],%B,[hideidle(%0)])]

@exitformat ROOM=[setq(lkr,%#)][setq(exits,[lvexits(%L,%#)])][setq(paths,[iter(%q<exits>,if(hasflag(##,build),,##))])][setq(build,[iter(%q<exits>,if(hasflag(##,build),##))])][u(exitformat`[gameconfig(%#,ExitFORMAT)],%q<paths>,%q<build>)]

&exitformat`default room=[if(gte(words(%q<exits>),1),[printf($-40s$-40s,[ansi([gameconfig(%#,line_color)],[chr(91)])][ansi([gameconfig(%#,column_names)],Paths)][ansi([gameconfig(%#,line_color)],[chr(93)])],[ansi([gameconfig(%#,line_color)],[chr(91)])][ansi([gameconfig(%#,column_names)],Buildings)][ansi([gameconfig(%#,line_color)],[chr(93)])])][setq(row,[max(words(%q<paths>),words(%q<build>))])][if(gte(%q<row>,1),%R[iter([lnum(1,%q<row>)],[printf($-40s$-40s,[u(exitformat`default`fmt,%q<lkr>,##,%q<paths>)],[u(exitformat`default`fmt,%q<lkr>,##,%q<build>)])],%B,%R)])]%R)][line(,%#)]

&exitformat`default`fmt room=[setq(ex,extract(%2,%1,1))][if(gte(words(%q<ex>),1),[printf($1s $-5s $-32s,,[ansi([gameconfig(%#,line_color)],[chr(91)])][u(get_exit`alias,%q<ex>,%q<lkr>)][ansi([gameconfig(%#,line_color)],[chr(93)])],[u(get_exit`name,%q<ex>,%q<lkr>)])])]

&get_exit`alias room=[setq(alias,[ucstr([extract(fullname(%0),2,1,;)])])][if(hasflag(%0,dark),[ansi([gameconfig(%#,DARK_EXITS)],%q<alias>)],[ansi([gameconfig(%#,line_text)],%q<alias>)])]
&get_exit`name room=[if(hasflag(%0,dark),[ansi([gameconfig(%#,DARK_EXITS)],name(%0))],name(%0))]
